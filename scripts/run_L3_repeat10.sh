...
# run_L3_repeat10.sh
#
# Repeat the `L3_post_compromise_security` lemma ten times on the final model and
...
#
# Output layout:
#   results/clean_release/L2_L3/L3_repeat10.csv
#   results/clean_release/L2_L3/L3_run_{1..10}.log
#   results/clean_release/L2_L3/L3_run_{1..10}.time
...
for rep in $(seq 1 10); do
  LOG="$OUT_DIR/L3_run_${rep}.log"
  TIME_FILE="$OUT_DIR/L3_run_${rep}.time"
  ...
  echo "L3 rep ${rep}: verdict=${VERDICT} steps=${STEPS} wall=${WALL}s rss=${RSS}KB"
done
...
} > "$OUT_DIR/postflight_integrity_L3.txt"
...
echo "L3 repeat10 finished; CSV at $CSV"
